...
const ProjectCard = memo(forwardRef(({ index, project, clothespinTexture, currentScroll, materials, curve, isSelected, scrollToIndex, onClick, isMobile, isTransitioning, paintProgress, roomOrigin }, ref) => {
    ...
    useFrame((state) => {
        ...
        if (isAnimating || isSelected || !currentScroll || typeof currentScroll.current !== "number" || isNaN(currentScroll.current) || !curve || typeof curve.getPointAt !== "function") return;
        ...
        if (isNaN(safeU) || !isFinite(safeU)) return;
        const pointOnCurve = curve.getPointAt(safeU);
        if (!pointOnCurve || typeof pointOnCurve.x !== "number") return;
